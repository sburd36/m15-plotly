...
cancer.rates <- read.csv("./data/IHME_WASHINGTON_MORTALITY_RATES_1980_2014.csv", stringsAsFactors = FALSE)

# Remove all non-county observations from the data set 
cancer.rates <- filter(cancer.rates, grepl("County", location_name))

# Remove all observations that contain mortality rates for "both" sex
cancer.rates <- filter(cancer.rates, !grepl("Both", sex))
...
King.County <- filter(cancer.rates, grepl("King County", location_name))
...
plot_ly(data = neoplasms.kc, x=~year_id, y=)
